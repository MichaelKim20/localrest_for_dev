...
if (isSpawnable!(F, T))
{
    ...
    else if (mode == SPAWN_MODE.ThreadMode)
    {
        void execInThread ()
        {
            thisInfo.ident = spawnTid;
            thisInfo.owner = ownerTid;
            thisInfo.scheduler = new NodeScheduler();
...
private:
    static class InfoFiber : Fiber
    {
        ...
        this (void delegate () op, ThreadInfo ownerInfo) nothrow
        {
            info.ownerScheduler = ownerInfo.scheduler;
            info.scheduler = ownerInfo.scheduler;
            info.have_scheduler = false;
            super(op);
        }
    }
