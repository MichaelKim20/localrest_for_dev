...
if (isSpawnable!(F, T))
{
    ...
    if (mode == SPAWN_MODE.ThreadMode)
    {
        void execInThread()
        {
            thisInfo.scheduler = new NodeScheduler(thisInfo);
    ...
    else
    {
        ...
        void execInFiber()
        {
            thisInfo.ident = spawnTid;
            thisInfo.owner = ownerTid;
            thisInfo.ownerScheduler = ownerScheduler;
...
struct ThreadInfo
{
    ...
    ***************************************************************************/
    ...
    void cleanup ()
    {
        ...
        if (scheduler !is null)
        {
            writefln("stop %s %s",scheduler.ownerInfo, thisInfo);
            if (scheduler.ownerInfo == thisInfo)
                scheduler.stop();
        }
...
{
    ThreadInfo ownerThreadInfo;

    this (ThreadInfo owner = ThreadInfo.init)
    {
        this.ownerThreadInfo = owner;
    }
