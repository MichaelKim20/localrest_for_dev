...
Tid spawnThread (F, T...) (F fn, T args)
if (isSpawnable!(F, T))
{
    static assert(!hasLocalAliasing!(T), "Aliases to mutable thread-local data not allowed.");
    return _spawn(SPAWN_MODE.ThreadMode, false, fn, args);
}

Tid spawnFiber (F, T...) (F fn, T args)
if (isSpawnable!(F, T))
{
    static assert(!hasLocalAliasing!(T), "Aliases to mutable thread-local data not allowed.");
    return _spawn(SPAWN_MODE.FiberMode, false, fn, args);
}
...
if (isSpawnable!(F, T))
{
    ...
    return _spawn(SPAWN_MODE.OriginalMode, true, fn, args);
}
...
private Tid _spawn (F, T...) (SPAWN_MODE mode, bool linked, F fn, T args)
if (isSpawnable!(F, T))
{
    ...
    if (mode == SPAWN_MODE.OriginalMode)
    {
        void exec ()
        {
    ...
    else if (mode == SPAWN_MODE.ThreadMode)
    {
        void execInThread ()
        {
            auto ownerScheduler = new AutoDispatchScheduler(dur!"msecs"(1));

            thisInfo.ident = spawnTid;
            thisInfo.owner = ownerTid;
            thisInfo.scheduler = ownerScheduler;
            thisInfo.have_scheduler = true;

            thisInfo.scheduler.spawn({
                thisInfo.owner = spawnTid;
                thisInfo.scheduler = ownerScheduler;
    ...
    {
        auto ownerScheduler = thisInfo.scheduler;
        ...
        if (ownerScheduler !is null)
        {
            ownerScheduler.spawn(&execInFiber);
        }
        else
        {
            if (thisInfo.scheduler !is null)
            {
                thisInfo.scheduler.spawn(&execInFiber);
            }
            else
            {
                thisInfo.scheduler = new AutoDispatchScheduler();
                thisInfo.have_scheduler = true;
                ownerScheduler = thisInfo.scheduler;
...
private void _send (T...) (MsgType type, Tid tid, T vals)
{
    ...
    else
    {
        ...
        //new Fiber({tid.mbox.put(msg);}).call();
/*
        if (scheduler is null)
...
struct ThreadInfo
{
    ...
    ***************************************************************************/
    ...
    void cleanup ()
    {
        ...
        if ((scheduler !is null) && have_scheduler)
            scheduler.stop();
...
class AutoDispatchScheduler : Scheduler
{
    private shared(bool) terminated;
    private Mutex m;
    private Duration sleep_interval;

    this (Duration sleep = dur!"msecs"(10))
    {
        this.sleep_interval = sleep;
        if (this.sleep_interval < dur!"msecs"(1))
            this.sleep_interval = dur!"msecs"(1);
        this.m = new Mutex();
...
private:
    ...
    void dispatch ()
    {
        ...
        while (!(terminated && (m_fibers.length == 0)))
        {
            m.lock();
