...
if (isSpawnable!(F, T))
{
    ...
    return _spawn(null, false, fn, args);
}

public Tid spawnThread (F, T...) (Scheduler scheduler, F fn, T args)
if (isSpawnable!(F, T))
{
    static assert(!hasLocalAliasing!(T), "Aliases to mutable thread-local data not allowed.");
    return _spawn(scheduler, false, fn, args);
}
...
if (isSpawnable!(F, T))
{
    ...
    return _spawn(null, true, fn, args);
}
...
private Tid _spawn (F, T...) (Scheduler scheduler, bool linked, F fn, T args)
if (isSpawnable!(F, T))
{
...
unittest
{
    ...
    auto spawnedTid = spawnThread(null, process);
    spawnedTid.send(42);
