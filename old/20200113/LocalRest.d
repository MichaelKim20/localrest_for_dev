    ...
    ***************************************************************************/
    ...
    private static Transceiver spawned (Implementation) (CtorParams!Implementation cargs)
    {
        ...
        thread_scheduler.spawn({
            ...
            thisScheduler.start({
                //writefln("ThreadScheduler.start");
...
private class Client
{
    ...
    private LocalFiberScheduler _scheduler;
    ...
    ***************************************************************************/
    ...
    public Response router (Transceiver remote, string method, string args) @trusted
    {
        ...
        {
            writefln("router 3      %s %s", method, args);
            req = Request(thisTransceiver, thisWaitingManager.getNextResponseId(), method, args);
            remote.send(req);
            return thisWaitingManager.waitResponse(req.id, this._timeout);
        }
        // from Non-Node to Node
        else
        {
            writefln("router 1 or 2 %s %s", method, args);
            if (thisScheduler is null)
                thisScheduler = new LocalFiberScheduler();

            //writefln("router ----- 2", method, args);
            ...
            //writefln("router ----- 3", method, args);
            thisScheduler.spawn({
                while (!this._terminate)
                {
                    //writefln("router ----- 3 - 1", method, args);
                    Message msg = this._transceiver.receive();
                    //writefln("router ----- 3 - 2", method, args);

                    if (this._terminate)
                        break;
                    //writefln("router ----- 3 - 3", method, args);

                    foreach (_; 0..10)
                    {
                        //writefln("router ----- 3 - 4", method, args);
                        if (this._waitingManager.exist(msg.res.id))
                            break;
                        //writefln("router ----- 3 - 5", method, args);
                        if (thisScheduler !is null)
                        {
            ...
            //writefln("router ----- 4", method, args);
            Response res;
            bool done = false;
            thisScheduler.spawn({
                //writefln("router ----- 4 - 1", method, args);
                res = this._waitingManager.waitResponse(req.id, this._timeout);
                //writefln("router ----- 4 - 2", method, args);
