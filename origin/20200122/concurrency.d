...
@safe unittest
{
    static struct Container { Transceiver t; }
    static assert(!hasLocalAliasing!(Transceiver, Container, int));
}
...
public class Transceiver
{
    ...
    {
        chan = new Channel(64*1024);
    }

    public void send (T) (T val) @trusted
    {
        this._send (MsgType.standard, val);
    }

    public void _send (T) (MsgType type, T val) @trusted
    {
        auto msg = Message (type, val);
        this.chan.send (msg);
    }
...
public class Channel
{
    ...
    private DList!(ChannelContext) sendq;

    /// collection of recv waiters
    private DList!(ChannelContext) recvq;

    /// Ctor
    public this (size_t qsize = 0) @safe nothrow
    {
        this.closed = false;
        this.mutex = new Mutex;
        this.qsize = qsize;
    ...
    ***************************************************************************/
    ...
    private bool _send (Message msg)
    {
        this.mutex.lock();
        ...
        {
            ChannelContext context = this.recvq.front;
    ...
    ***************************************************************************/
    ...
    private bool _receive (Message* msg)
    {
        this.mutex.lock();
        ...
        {
            ChannelContext context = this.sendq.front;
